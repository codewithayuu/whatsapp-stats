...
const UploadIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-8 h-8 text-teal-300">
...
export default function App() {
  ...
  const processChatText = (text) => {
    ...
    const finalAnalytics = {
        ...
        firstMessages: Array.from(intermediateStats.firstMessage.entries()).map(([name, {date, message}]) => ({ name, date, message })).sort((a,b) => parseWhatsAppDate(a.date) - parseWhatsAppDate(b.date)),
        lastMessages: Array.from(intermediateStats.lastMessage.entries()).map(([name, {date, message}]) => ({ name, date, message })).sort((a,b) => parseWhatsAppDate(b.date) - parseWhatsAppDate(a.date)),
  ...
  const handleFile = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      } catch (err) { console.error("Processing error:", err); setError(err.message); } 
